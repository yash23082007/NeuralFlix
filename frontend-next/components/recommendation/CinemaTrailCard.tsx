...
export default function CinemaTrailCard({ trail }: { trail: CinemaTrail }) {
    ...
    <div className="group relative overflow-hidden rounded-3xl border border-border bg-surface p-6 sm:p-8 transition-all hover:border-accent/30 hover:shadow-2xl hover:shadow-accent/5">
      ...
      <div className="mb-8 max-w-2xl">
        <div className="flex items-center gap-2 text-text-muted mb-3">
          ...
          <span className="text-xs font-bold uppercase tracking-wider">{trail.region}</span>
        </div>
      ...
      <div className="relative mt-12 pb-4">
        ...
        <div className="flex flex-col gap-12 sm:flex-row sm:gap-4 sm:justify-between relative z-10">
          {trail.movies.map((movie, idx) => {
            const nextTransition = trail.transitionReasons.find(r => r.from === movie.title);
            
            return (
              <div key={movie.tmdb_id} className="relative flex items-start gap-6 sm:flex-col sm:items-center sm:gap-4 sm:w-1/5 group/node">
                {/* Node Dot */}
                ...
                {/* Transition Reason Text (Mobile: below title, Desktop: tooltip or below) */}
                {nextTransition && (
